...
Future<void> showEditKelasPopup(
    {required List<Department>? department,
    required mayor.Mayor? mayor,
    required BuildContext context,
    required Function(Map<String, dynamic> data) onSubmit}) async {
  final GlobalKey<FormState> formKey = GlobalKey<FormState>();
  final TextEditingController nameController = TextEditingController();
  nameController.text = mayor?.nama ?? '';
  final listJurusan = department?.toList() ?? [];
  int? selectedJurusan =  mayor?.department?.id;
  // String? jurusan = mayor?.department?.nama ?? '';
  showDialog(
    context: context,
